// views/config.js — read-mostly view of the server config.
...
function renderField(label, name, value, help, opts = {}) {
  return `
...
function readTimeoutsFromInputs() {
  const fields = ["connect_ms", "request_send_ms", "ttft_ms", "idle_chunk_ms", "total_ms"];
  const out = {};
  for (const f of fields) {
    const el = document.querySelector(`input[name="timeouts.${f}"]`);
...
async function configSaveTimeouts() {
