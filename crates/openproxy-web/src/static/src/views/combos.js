// views/combos.js — combo grid + combo detail (with target table).
//
...
async function renderComboDetail(comboId) {
  ...
  if (!combo) {
    document.getElementById("main").innerHTML = `<div class="banner banner-error">Combo ${comboId} not found. <a href="#/combos">← Back</a></div>`;
    return;
  }
  ...
  `;
  let body = "";
  if (state.comboTestResults[comboId]) body += renderComboTestResults(state.comboTestResults[comboId]);
  body += `
...
export async function mountCombos(opts = {}) {
