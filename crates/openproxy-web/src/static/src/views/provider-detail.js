// views/provider-detail.js — the provider detail view. This is
...
import { renderModelRows, getVisibleModelRowIds, updateFilterTabCounts, syncSelectAllCheckbox, SORTABLE_COLUMNS, applySort, renderSortableTh } from "../components/model-table.js";
import { BUILTIN_PROVIDER_IDS, OAUTH_PROVIDER_IDS, OAUTH_PKCE_PROVIDERS, OAUTH_DEVICE_CODE_PROVIDERS, providerHasQuota } from "../lib/constants.js";
...
function getProviderIconHtml(providerId, format) {
  const knownLogos = { openrouter: "🟢", minimax: "🟡", "opencode-zen": "🟣" };
...
function renderOAuthSection(provider) {
...
function renderConnectionsSection(provider, accounts) {
...
function renderModelsSection(provider, providerModels, activeModels, ui) {
...
export async function renderProviderDetail(providerId) {
  ...
  document.getElementById("main").innerHTML = header + body;
