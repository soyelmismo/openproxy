// views/home.js — landing view. Summarises the current state of
...
export async function mountHome() {
  ...
  // what it needs at mount time, just like providers.js does.
  ...
  const [providers, accounts, models, combos, keys, recent] = await Promise.all([
    ...
    (state.combos    && state.combos.length)    ? Promise.resolve(state.combos)    : api("/combos"),
    (state.apiKeys   && state.apiKeys.length)   ? Promise.resolve(state.apiKeys)   : api("/keys"),
    api("/usage/recent?limit=5").catch(() => []),
  ]);
  ...
  // paint rather than "—".
  if (!state.health) {
    try { state.health = await api("/health"); } catch (_) { /* keep null */ }
  }
  const summary = {
