// views/provider-grid.ts — the providers landing page. Renders a
...
type StatsByProvider = Record<string, ProviderStats | undefined>;

function computeProviderStats(): StatsByProvider {
  const stats: StatsByProvider = {};
...
function renderProviderCard(p: Provider, s: ProviderStats | undefined): string {
  // s is the rollup for p; if the caller didn't pre-compute one
  // (shouldn't happen for the live path, but possible from tests)
  // we fall back to zeroed stats so the card still renders.
  ...
  return `
    <a href="#/providers/${encodeURIComponent(p.id)}" class="${cardClasses}">
      ...
      <div class="provider-card-footer">
        ...
        <div class="stat">
          <label>Models</label>
          <value>${s?.active_models ?? 0}/${models.length}</value>
        </div>
      </div>
    </a>
  `;
...
export function renderProviderGrid(): void {
