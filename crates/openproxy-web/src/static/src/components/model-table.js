// components/model-table.js — render the inner HTML of the
...
export function modelStatusPillClass(status) {
...
export function renderCapabilityBadges(json) {
...
export function getVisibleModelRowIds() {
...
export function updateFilterTabCounts(providerId, allProviderModels) {
...
export function syncSelectAllCheckbox(visibleRowIds) {
  ...
  const selectedVisible = visibleRowIds.filter((id) => state.selectedModels.has(id)).length;
...
export function applySort(rows, sort) {
  ...
  const col = SORTABLE_COLUMNS.find((c) => c.key === sort.key);
  if (!col) return rows;
...
export function renderSortableTh(col, sort, providerId) {
