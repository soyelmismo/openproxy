// handlers/combo-handlers.js — combo CRUD: create, delete, race
// size, test-all. Target-specific actions live in
// combo-target-handlers.js.
//
...
export function showCreateCombo() {
  const html = `
...
export function closeCreateCombo() {
  const m = document.getElementById("create-combo-modal");
  if (m) m.remove();
}

export async function createCombo(e) {
...
export async function updateRaceSize(id, e) {
  const val = e && e.target ? parseInt(e.target.value, 10) : NaN;
...
export async function testAllTargets(comboId, e) {
  ...
  try {
    ...
    alert("Test all failed: " + (err.message || err));
  } finally {
