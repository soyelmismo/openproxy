// tests/e2e/sidebar-collapse.spec.js
...
//     to the right link)
//
// Run with: npx playwright test sidebar-collapse.spec.js
...
test('sidebar collapse to icons + localStorage persistence', async ({ page }) => {
  // 1. Start clean.
  ...
  const labelAfter = await page.locator('.sidebar nav a .nav-label').first();
  ...
  await page.reload();
  const storedAfterReload = await page.evaluate((k) => localStorage.getItem(k), STORAGE_KEY);
  ...
  await page.locator('.sidebar-toggle').click();
  const storedExpanded = await page.evaluate((k) => localStorage.getItem(k), STORAGE_KEY);
  ...
  await page.reload();
  const storedExpandedReload = await page.evaluate((k) => localStorage.getItem(k), STORAGE_KEY);
...
test('nav link click + active class works in both collapsed and expanded modes', async ({ page }) => {
  await page.goto(BASE);
  await page.evaluate((k) => localStorage.removeItem(k), STORAGE_KEY);
  await page.reload();
