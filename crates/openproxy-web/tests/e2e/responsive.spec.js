// tests/e2e/responsive.spec.js — verifies the base responsive
...
const NAV_LINKS = ['Home', 'Providers', 'Combos', 'API Keys', 'Analytics', 'Live Logs', 'Config'];
...
test.describe('Responsive — mobile (375x667)', () => {
  ...
  test('home: sidebar is a top-bar, no horizontal body scroll, all nav links visible', async ({ page }) => {
    await gotoHome(page);
  ...
  test('logs: table is horizontally scrollable inside main, not on body', async ({ page }) => {
    await gotoHome(page);
  ...
  test('480px: main padding is reduced, page-header h2 is 1.1rem', async ({ page }) => {
...
test.describe('Responsive — desktop (1280x800)', () => {
  ...
  test('desktop layout intact: sidebar is left column, no regression', async ({ page }) => {
    await gotoHome(page);
  ...
  test('desktop: sidebar collapse toggle still works (regression check)', async ({ page }) => {
    // Start from a clean state — make sure the user hasn't left
    // the sidebar collapsed from a previous test.
    await page.goto('http://localhost:8788/');
    await page.evaluate(() => {
      try { localStorage.removeItem('openproxy:sidebarCollapsed'); } catch (_) {}
    });
    await gotoHome(page);
    ...
    expect(collapsedWidth).toBeLessThan(200);
    ...
    const reExpandedWidth = (await sidebar.boundingBox()).width;
    expect(reExpandedWidth).toBe(200);
  });
});
