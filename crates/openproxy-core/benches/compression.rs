...
use openproxy_core::compression::{CompressionMode, apply_compression};
...
fn build_fixture() -> Vec<openproxy_core::translation::OpenAIMessage> {
    ...
    messages.push(openproxy_core::translation::OpenAIMessage {
        role: "user".to_string(),
        content: Some(Value::String(
            "Run `git status` and tell me what changed.".to_string(),
        )),
        name: None,
        tool_call_id: None,
        tool_calls: None,
    ...
    cargo_test.push_str(
        "\x1b[1m\x1b[32m    Finished test [unoptimized + debuginfo] target(s) in 0.52s\x1b[0m\n",
    );
    ...
    cargo_test.push('\n');
    cargo_test.push_str(
        "test result: ok. 25 passed; 0 failed; 0 ignored; 0 measured; 100% filtered out\n",
    );
...
fn bench_compression(c: &mut Criterion) {
    ...
    group.bench_function("lite", |b| {
        b.iter_with_setup(build_fixture, |mut msgs| {
