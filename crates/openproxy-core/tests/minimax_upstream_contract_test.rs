...
fn test_minimax_golden_contract_spec_parity() {
    ...
    assert_eq!(
        MiniMaxRegion::China.account_origin(),
        "https://account.minimax.cn"
    );
    assert_eq!(
        MiniMaxRegion::Global.gateway_origin(),
        "https://agent.minimax.io"
    );
    assert_eq!(
        MiniMaxRegion::China.gateway_origin(),
        "https://agent.minimax.cn"
    );
    assert_eq!(
        MiniMaxRegion::Global.platform_origin(),
        "https://platform.minimax.io"
    );
    assert_eq!(
        MiniMaxRegion::China.platform_origin(),
    ...
    assert!(
        url.contains("user_code=LRAR-BW2A"),
        "must contain user_code"
    );
    assert!(
        url.contains("client_surface=tui"),
        "must attribute client_surface=tui"
    );
    assert!(
        url.contains("download_source=mcode-internal"),
    ...
    let model_map: std::collections::HashMap<_, _> = builtin_models
        ...
        .map(|m| {
            (
                m.model_id.as_str(),
                (m.context_length, m.max_output_tokens, m.target_format),
            )
        })
        .collect();
...
async fn test_minimax_remote_upstream_live_contract_parity() {
    ...
    let resp = match client.call(req, TimeoutProfile::OAuth, cancel).await {
        ...
        Ok(r) => {
            eprintln!(
                "[ContractTest] Upstream probe HTTP {}, skipping live check",
                r.status
            );
            return;
        }
    ...
    let resp = client
        .call(
            UpstreamRequest::get(&endpoint_cfg_url),
    ...
    let resp = client
        .call(
            UpstreamRequest::get(&auth_url_ts_url),
            TimeoutProfile::OAuth,
            CancellationToken::new(),
        )
        .await
        .expect("fetch authorization-url.ts");
    let auth_url_bytes = resp
        .collect()
        .await
        .expect("read authorization-url.ts body");
    ...
    let resp = client
        .call(
            UpstreamRequest::get(&http_gateway_url),
            TimeoutProfile::OAuth,
            CancellationToken::new(),
        )
        .await
    ...
    let resp = client
        .call(
            UpstreamRequest::get(&daily_signin_url),
            TimeoutProfile::OAuth,
            CancellationToken::new(),
        )
        .await
    ...
    let resp = client
        .call(
            UpstreamRequest::get(&matrix_client_url),
            TimeoutProfile::OAuth,
            CancellationToken::new(),
        )
        .await
        .expect("fetch matrix-account-client.ts");
    let matrix_bytes = resp
        .collect()
        .await
        .expect("read matrix-account-client.ts body");
    ...
    assert!(
        matrix_ts.contains("device_platform: 'mcode'")
            || matrix_ts.contains("device_platform: \"mcode\""),
    ...
    let resp = client
        .call(
            UpstreamRequest::get(&llm_api_url),
            TimeoutProfile::OAuth,
            CancellationToken::new(),
        )
        .await
    ...
    let resp = client
        .call(
            UpstreamRequest::get(&config_ts_url),
            TimeoutProfile::OAuth,
            CancellationToken::new(),
        )
        .await
    ...
    assert!(
        models_block.contains("\"MiniMax-M2.7-highspeed\":"),
        "upstream missing MiniMax-M2.7-highspeed"
    );
    assert!(
        models_block.contains("\"MiniMax-M2.7\":"),
        "upstream missing MiniMax-M2.7"
    );
    assert!(
        models_block.contains("contextWindowOptions: [512000, 1000000]"),
        "MiniMax-M3 contextWindowOptions diverged"
    );
    assert!(
        models_block.contains("output: 128000"),
        "output token ceiling diverged"
    );
    assert!(
        models_block.contains("context: 200000"),
    ...
    let resp = client
        .call(
            UpstreamRequest::get(&helpers_ts_url),
            TimeoutProfile::OAuth,
            CancellationToken::new(),
        )
        .await
        .expect("fetch model-resolver-helpers.ts");
    let helpers_bytes = resp
        .collect()
        .await
        .expect("read model-resolver-helpers.ts body");
    ...
    let adapter = openproxy_adapters::adapters::minimax::MiniMaxAdapter::new();
    let model = openproxy_types::ModelId::new("MiniMax-M3");
    let our_headers = adapter.build_headers(
        "test-token-123",
        openproxy_types::TargetFormat::Anthropic,
        &model,
    );
    ...
    assert!(
        scanned_headers.contains(&"X-Mavis-Session-Id".to_string()),
        "Scanner must find X-Mavis-Session-Id"
    );
    assert!(
        scanned_headers.contains(&"X-Mavis-Agent-Id".to_string()),
        "Scanner must find X-Mavis-Agent-Id"
    );
    assert!(
        scanned_headers.contains(&"X-Mavis-Timezone-Offset".to_string()),
        "Scanner must find X-Mavis-Timezone-Offset"
    );
    assert!(
        scanned_headers.contains(&"User-Agent".to_string()),
    ...
    for upstream_header in &scanned_headers {
        assert!(
            our_headers
                .iter()
                .any(|(k, _)| k.eq_ignore_ascii_case(upstream_header)),
...
fn test_minimax_dynamic_spoofer_and_endpoint_resolution() {
    ...
    unsafe {
        std::env::set_var(
            ...
            "https://mock-account.local",
        );
        std::env::set_var(
            "OPENPROXY_MINIMAX_GATEWAY_BASE_URL",
            "https://mock-agent.local",
        );
    }
