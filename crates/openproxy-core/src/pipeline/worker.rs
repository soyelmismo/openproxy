...
use crate::combos::{SelectionRegistry, CooldownMode};
...
pub fn process_job(conn_clone: &Arc<parking_lot::Mutex<Connection>>, job: BackgroundJob) {
    match job {
                            ...
                            match op {
                                ...
                                "record" => {
                                    ...
                                    if let Err(e) = crate::cooldown::record_failure_with_mode(
                                        &lock, target_id, &reason, cooldown_mode, cooldown_base_secs, cooldown_max_secs, cooldown_factor,
                                    ) {
                                        tracing::warn!("cooldown::record_failure_with_mode failed in background: {}", e);
                                    }
                                }
                                _ => {}
                            }
                    ...
                    BackgroundJob::MarkClientResponse { request_id, attempt, target_id } => {
                        let lock = conn_clone.lock();
