...
use crate::pipeline::streaming_state::{StreamContext, ChunkResult};
        ...
        let bytes = match stream.next_chunk().await {
            ...
            Err(e) => {
                let err = match e {
                    ...
                    UpstreamError::Connection(msg) | UpstreamError::Tls(msg) | UpstreamError::Http(msg) | UpstreamError::Decode(msg) | UpstreamError::Invalid(msg) => {
                        CoreError::UpstreamConnection(format!("stream read: {}", msg))
                    }
                    UpstreamError::Timeout(_) => CoreError::UpstreamConnection(format!("stream read: {}", e)),
                };
                return Err(err);
            }
        };
