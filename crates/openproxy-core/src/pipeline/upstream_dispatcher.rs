...
use crate::pipeline::{FailureContext, PipelineRequest, PipelineResult, parse_retry_after_ms};
use crate::timeouts::Timeouts;
use crate::translation::OpenAIResponse;
use crate::upstream::{CancellationToken, UpstreamError, UpstreamRequest};
use std::sync::Arc;
...
impl UpstreamDispatcher {
    ...
    pub(crate) async fn dispatch_upstream(
        ...
        let openai_response = match target_format {
            crate::models::TargetFormat::Responses => {
    ...
    pub(crate) async fn dispatch_upstream_streaming(
        ...
        let mut state = crate::pipeline::streaming_state::StreamingState::new(true);

        let ctx = crate::pipeline::streaming_state::StreamContext {
            req: &req,
            combo,
            target,
            model,
        ...
        match state.run_stream_loop(&ctx, self, &mut stream).await {
            Ok(crate::pipeline::streaming_state::ChunkResult::Return(r)) => return r,
            Ok(crate::pipeline::streaming_state::ChunkResult::Break) => {}
            Err(e) => {
