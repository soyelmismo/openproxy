...
use crate::pipeline::FailureContext;
use crate::pipeline::{PipelineRequest, PipelineResult, SSE_DONE_BYTES};
            ...
            if let Some(choice) = choices.first_mut() {
                if let Some(delta) = &mut choice.delta {
                    if let Some(content) = delta.content.as_ref() {
                        ...
                        let has_native_reasoning = delta.reasoning_content.is_some() || delta.extra.contains_key("reasoning_content");
...
pub(crate) struct StreamContext<'a> {
    ...
    pub model: &'a Model,
    pub target_format: crate::models::TargetFormat,
...
impl StreamingState {
    ...
    pub(crate) async fn run_stream_loop(
        &mut self,
        ctx: &StreamContext<'_>,
        dispatcher: &crate::pipeline::upstream_dispatcher::UpstreamDispatcher,
        ...
        let mut processor = ChunkProcessor { state: self, dispatcher };
        let pipeline_result = crate::pipeline::streaming::pipeline::run_pipeline(ctx, stream, sse_parser, &mut processor).await?;
        if let ChunkResult::Return(_) = pipeline_result {
            return Ok(pipeline_result);
        }
        
        // Cancellation checkpoint
        if ctx.req.race_cancel.as_ref().is_some_and(|rc| rc.is_cancelled()) && !processor.state.done_sent {
            return Ok(ChunkResult::Return(dispatcher.fail_stream_client_disconnected(
...
impl<'a> crate::pipeline::streaming::ChunkInterceptor for ChunkProcessor<'a> {
    async fn process_chunk(&mut self, ctx: &StreamContext<'_>, stream: &mut crate::upstream::UpstreamBodyStream, event: crate::pipeline::streaming::ChunkEvent) -> Result<crate::pipeline::streaming::ChunkEvent, CoreError> {
        let crate::pipeline::streaming::ChunkEvent::Data(line_bytes) = event else {
            return Ok(event);
        };
        let state = &mut *self.state;

        if state.done_sent {
            return Ok(crate::pipeline::streaming::ChunkEvent::Done);
        }
        ...
        if ctx.req.race_cancel.as_ref().is_some_and(|rc| rc.is_cancelled()) {
            return Ok(crate::pipeline::streaming::ChunkEvent::Return(self.dispatcher.fail_stream_client_disconnected(
                crate::pipeline::upstream_dispatcher::StreamFailureContext {
        ...
        // ── Format dispatch ──
        if ctx.target_format == crate::models::TargetFormat::Openai {
            self.process_openai_format(ctx, stream, line, &line_bytes).await
        } else {
...
impl<'a> ChunkProcessor<'a> {
    ...
    async fn process_openai_format(
        ...
        line: &str,
        line_bytes: &[u8],
    ) -> Result<crate::pipeline::streaming::ChunkEvent, CoreError> {
        ...
        let connect_and_send_ms = ctx.connect_and_send_ms;
                            ...
                            if json_payload == "[DONE]" {
                                ...
                                if let Err(crate::race_sink::StreamSinkError::Lost) = sink.send(SSE_DONE_BYTES.clone()).await {
                                    return Ok(crate::pipeline::streaming::ChunkEvent::Return(self.dispatcher.fail_on_sink_send_error(
                            ...
                            if json_payload.contains("\"error\":")
                                ...
                                if let Ok(ec) = serde_json::from_str::<ErrorChunk>(json_payload) {
                                    ...
                                    if has_empty_choices && let Some(error_obj) = ec.error {
                                        ...
                                        let message = error_obj.message.unwrap_or("unknown upstream error in SSE stream");
                                        ...
                                        let acc_ref: Option<&crate::sse_accumulator::ResponseAccumulator> =
                                            ...
                                            return Ok(crate::pipeline::streaming::ChunkEvent::Return(pipeline.record_and_fail_with_trace_id_and_partial(
                                                Arc::clone(req),
                                                combo,
                                                target,
                                                FailureContext {
    ...
    async fn process_translated_format(
        ...
        let connect_and_send_ms = ctx.connect_and_send_ms;
                        ...
                        match parsed {
                            ...
                            Ok(None) => return Ok(crate::pipeline::streaming::ChunkEvent::Skip),
                            Err(e) => {
                                tracing::warn!(
                                    chunk_id = %chunk_id,
                                    error = %e,
