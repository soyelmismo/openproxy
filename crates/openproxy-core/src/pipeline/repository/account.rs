...
pub fn get_accounts_meta(
    ...
    let query = format!(
        "SELECT id, api_key_encrypted, label, access_token_encrypted, refresh_token_encrypted, expires_at, oauth_provider_specific, quota_session_reset_at, quota_model_details FROM accounts WHERE id IN ({})",
        placeholders
    );
