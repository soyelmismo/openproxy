...
//!
//! Extracted from the monolithic `models.rs` to keep the production
...
fn apply_auto_activation_does_not_affect_old_re_upserted_model() {
    ...
    let updated = apply_auto_activation(&conn, &provider, Some("claude")).expect("apply keyword");
    assert_eq!(
        updated, 0,
...
fn delete_model_sets_combo_target_model_row_id_to_null() {
    ...
    assert_eq!(
        ...
        Some(m_id.0),
        "T is the row pointing at M pre-delete"
    );
    assert_eq!(
    ...
    assert_eq!(
        query_combo_target_model_row_id(&conn, combo_id, m_id.0),
        None,
        "T was cascade-deleted with model M"
    );
