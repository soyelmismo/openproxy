...
impl CancellationToken {
    ...
    /// dropping the token does NOT stop the task (it lives until the
    /// watch is dropped). Use this in `Gate 1+` call sites that already
    /// have a `client_disconnected: watch::Receiver<bool>` and want to
    /// plug it into the upstream client's cancel slot.
