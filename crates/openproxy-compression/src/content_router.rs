...
static SEARCH_RESULT_RE: LazyLock<Regex> =
    LazyLock::new(|| openproxy_types::static_regex!(r"^[\w/.\-]+:\d+:"));
...
static SOURCE_IMPORT_RE: LazyLock<Regex> = LazyLock::new(|| {
    openproxy_types::static_regex!(r"^(import |from |use |package |#include |require\()")
});
...
static GENERIC_ERROR_RE: LazyLock<Regex> = LazyLock::new(|| {
    openproxy_types::static_regex!(r"(?i)(error|fail|warn|traceback|panic|exception)")
});

/// `^make[N]:` (N is digits).
static MAKE_RE: LazyLock<Regex> =
    LazyLock::new(|| openproxy_types::static_regex!(r"^make\[\d+\]:"));
