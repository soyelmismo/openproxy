...
impl ProviderAdapter for OpenCodeGoAdapter {
    ...
    async fn fetch_models(
        ...
        let url = self
            .models_url()
            .ok_or_else(|| CoreError::Validation("opencode-go: models_url is None".into()))?;
        ...
        let payload: OpenAIModelsResponse = <OpenAIModelsResponse as serde::Deserialize>::deserialize(&body)
        ...
        let out = payload
            ...
            .map(|m| {
                let id = m.id;
                let target_format = classify_go_target_format(&id);
                DiscoveredModel {
...
/// case-insensitive.
pub(crate) fn classify_go_target_format(id: &str) -> TargetFormat {
