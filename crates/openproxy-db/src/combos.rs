...
fn update_combo_column<T: rusqlite::ToSql>(
    ...
    let column_name = column.as_str();
    let affected = conn.execute(column.sql(), params![value, id.0]).map_err(
        crate::error::map_db_error_ctx(format!("update {column_name} for combo {}", id.0)),
    )?;
    if affected == 0 {
        return Err(CoreError::ComboNotFound(id.0));
    }
...
pub fn update_cooldown_factor(conn: &Connection, id: ComboId, factor: Option<u32>) -> Result<()> {
    update_combo_column(conn, id, ComboColumn::CooldownFactor, factor.map(i64::from))
}
