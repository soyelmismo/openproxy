-- 000069_remove_fx_provider_and_format.sql
-- Remove 'fx' from the providers.format and models.target_format CHECK constraints and clean up fx rows.
...
CREATE TABLE providers_new (
  ...
  CHECK (format IN ('openai', 'anthropic', 'mixed', 'gemini', 'responses', 'atomesus'))
);
...
INSERT INTO providers_new (
  ...
  rate_limit_scope, proxy_rotation_mode, favicon_base64
)
SELECT
  id, name, base_url, auth_type, format,
  extra_headers_json, auto_activate_keyword, use_proxies,
...
CREATE TABLE models_new (
  ...
  CHECK (target_format IN ('openai', 'anthropic', 'gemini', 'responses', 'atomesus'))
);
...
SELECT
  ...
  manually_disabled_at
FROM models
WHERE provider_id != 'fx' AND target_format != 'fx';
...
-- Clean up any related targets or accounts referencing 'fx'
DELETE FROM combo_targets WHERE provider_id = 'fx';
DELETE FROM accounts WHERE provider_id = 'fx';
